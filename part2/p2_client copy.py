...
class ReliableUDPClient:
    def __init__(self, server_ip, server_port, prefix):
        ...
        self.sack_blocks_sent = 0  # Total SACK blocks sent across all ACKs
        self.duplicate_ack_sent_count = 0
    ...
    def log_stats(self):
        ...
        stats = {
            ...
            'ack_sent_count': self.ack_sent_count,
            'sack_blocks_sent': self.sack_blocks_sent,
            'duplicate_ack_sent_count': self.duplicate_ack_sent_count,
            'timeout_count': self.timeout_count,
            'total_packets_received': self.total_packets_received,
            'out_of_order_packets': self.out_of_order_packets,
            'sack_usage_rate': (self.sack_blocks_sent / max(self.ack_sent_count, 1)) if self.ack_sent_count > 0 else 0
        }
        with open('client_stats.json', 'w') as f:
            json.dump(stats, f, indent=4)
        print(f"[Client] Statistics logged to client_stats.json: {stats}")
    ...
    def make_ack(self, ack_num):
        ...
        self.sack_blocks_sent += len(sack_blocks)
        return ack_packet
    ...
    def send_ack(self, ack_num, is_duplicate=False):
        ...
        if is_duplicate:
            self.duplicate_ack_sent_count += 1
